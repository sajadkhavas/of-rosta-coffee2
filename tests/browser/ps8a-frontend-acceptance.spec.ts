...
test("PS8A proves production-indexable public SSR metadata and canonical policy", async ({ page }, testInfo) => {
  ...
  for (const expected of publicSeoRoutes) {
    ...
    expect(actual.title.trim().length, `${expected.route} must expose a meaningful title`).toBeGreaterThan(8);
    expect(
    ...
    expect(actual.xRobotsTag ?? "", `${expected.route} must not receive an HTTP noindex policy`).not.toContain(
      "noindex",
    );
