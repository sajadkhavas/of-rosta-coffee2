...
async function expectAccessible(page: import("@playwright/test").Page) {
  ...
  const violations = await page.evaluate(async () => {
    const axe = (window as unknown as { axe: { run: (context: Document) => Promise<{ violations: Array<{ impact: string | null; id: string }> }> } }).axe;
...
test.describe("PS5A persisted quiz journey", () => {
  test("guest completes server quiz without persisting answers in LocalStorage", async ({ page }) => {
    const response = await page.goto("/quiz");
    expect(response?.status()).toBe(200);
    await expect(page.locator('meta[name="robots"]')).toHaveAttribute("content", /noindex/i);
    await expect(page.getByRole("heading", { name: "با چه دستگاهی قهوه درست می‌کنی؟" })).toBeVisible();
    ...
    await expect(page.getByRole("heading", { name: "قهوه‌های هماهنگ با پاسخ‌های تو" })).toBeVisible();
