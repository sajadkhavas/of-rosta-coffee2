...
export async function listSellerReviews(roasteryId: string): Promise<SellerReviewSafety[]> {
  return parseContract(
    resourceSchema(listSellerSchema),
    await apiFetch<unknown>(`/seller/roasteries/${encodeURIComponent(roasteryId)}/reviews`),
    "نظرهای فروشنده",
  ).data.items;
}
export async function saveSellerReply(roasteryId: string, reviewId: string, body: string) {
  return parseContract(
    ...
    await apiFetch<unknown>(
      `/seller/roasteries/${encodeURIComponent(roasteryId)}/reviews/${encodeURIComponent(reviewId)}/reply`,
      { method: "PUT", body: { body: body.trim() } },
    ),
    "پاسخ فروشنده",
  ).data;
}
export async function listAdminReviewReports(
  status: ReviewAbuseStatus = "open",
): Promise<ReviewAbuseReport[]> {
  return parseContract(
    resourceSchema(listReportSchema),
...
export async function moderateReviewReport(
  reportId: string,
  status: ReviewAbuseStatus,
  resolutionReason?: string,
) {
  return parseContract(
    resourceSchema(reportSchema),
    await apiFetch<unknown>(`/admin/review-reports/${encodeURIComponent(reportId)}`, {
      method: "PATCH",
      body: { status, resolution_reason: resolutionReason?.trim() || null },
    }),
    "بررسی گزارش نظر",
  ).data;
}
export async function moderateSellerReply(
  replyId: string,
  ...
  return parseContract(
    resourceSchema(replySchema),
    await apiFetch<unknown>(`/admin/review-replies/${encodeURIComponent(replyId)}`, {
      method: "PATCH",
      body: { status, reason: reason?.trim() || null },
    }),
    "Moderation پاسخ فروشنده",
  ).data;
}
