...
const collection = <T extends z.ZodTypeAny>(item: T) => z.object({
...
const inquirySchema = z.object({
  id,
  type: z.enum(["support", "order_issue", "roastery_onboarding", "corporate_purchase", "content_correction", "privacy_request"]),
...
export const listAdminRoasteries = (status: AdminRoasteryStatus) => list(`/admin/roasteries?status=${status}&per_page=100`, roasterySchema, "روستری‌های ادمین");
export const listAdminProducts = (status: AdminProductStatus) => list(`/admin/products?status=${status}&per_page=100`, productSchema, "محصولات ادمین");
export const listAdminReviews = (status: AdminReviewStatus) => list(`/admin/reviews?status=${status}&per_page=100`, reviewSchema, "نظرات ادمین");
export const listAdminInquiries = (status: AdminInquiryStatus) => list(`/admin/inquiries?status=${status}&per_page=100`, inquirySchema, "درخواست‌های پشتیبانی");
export const listAdminAudits = (action = "") => list(`/admin/operations/audits?per_page=100${action ? `&action=${encodeURIComponent(action)}` : ""}`, auditSchema, "گزارش ممیزی");
export const listAdminNotifications = (status: AdminNotificationStatus) => list(`/admin/operations/notifications?status=${status}&per_page=100`, notificationSchema, "صف اعلان‌ها");

export async function setRoasteryStatus(value: string, status: AdminRoasteryStatus) {
  return parseContract(resourceSchema(z.unknown()), await apiFetch(`/admin/roasteries/${encodeURIComponent(value)}/status`, { method: "PATCH", body: { status } }), "تغییر وضعیت روستری").data;
}
export async function setProductStatus(value: string, status: AdminProductStatus) {
  return parseContract(resourceSchema(z.unknown()), await apiFetch(`/admin/products/${encodeURIComponent(value)}/status`, { method: "PATCH", body: { status } }), "تغییر وضعیت محصول").data;
}
export async function moderateReview(value: string, status: Exclude<AdminReviewStatus, "pending">, reason?: string) {
  return parseContract(resourceSchema(reviewSchema), await apiFetch(`/admin/reviews/${encodeURIComponent(value)}`, { method: "PATCH", body: { status, reason: reason?.trim() || undefined } }), "مدیریت نظر").data;
}
export async function setInquiryStatus(value: string, status: AdminInquiryStatus) {
  return parseContract(resourceSchema(inquirySchema), await apiFetch(`/admin/inquiries/${encodeURIComponent(value)}`, { method: "PATCH", body: { status } }), "تغییر وضعیت درخواست").data;
}

export const adminRoasteriesQuery = (status: AdminRoasteryStatus) => queryOptions({ queryKey: ["admin", "operations", "roasteries", status], queryFn: () => listAdminRoasteries(status), staleTime: 15_000 });
export const adminProductsQuery = (status: AdminProductStatus) => queryOptions({ queryKey: ["admin", "operations", "products", status], queryFn: () => listAdminProducts(status), staleTime: 15_000 });
export const adminReviewsQuery = (status: AdminReviewStatus) => queryOptions({ queryKey: ["admin", "operations", "reviews", status], queryFn: () => listAdminReviews(status), staleTime: 10_000 });
export const adminInquiriesQuery = (status: AdminInquiryStatus) => queryOptions({ queryKey: ["admin", "operations", "inquiries", status], queryFn: () => listAdminInquiries(status), staleTime: 10_000 });
export const adminAuditsQuery = (action = "") => queryOptions({ queryKey: ["admin", "operations", "audits", action], queryFn: () => listAdminAudits(action), staleTime: 10_000 });
export const adminNotificationsQuery = (status: AdminNotificationStatus) => queryOptions({ queryKey: ["admin", "operations", "notifications", status], queryFn: () => listAdminNotifications(status), staleTime: 10_000 });
