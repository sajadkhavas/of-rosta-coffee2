...
function operationsCollection<T extends z.ZodTypeAny>(item: T) {
  return z
    .object({
      data: z
        .object({
...
const roasteryStatusSchema = z.enum([
  "pending",
  "verified",
  "suspended",
  "rejected",
...
async function list<T>(path: string, schema: z.ZodType<T>, label: string): Promise<T[]> {
  ...
  return parseContract(operationsCollection(schema), raw, label).data.items;
}
...
export const listAdminAudits = (action = "") =>
  list(`/admin/operations/audits?per_page=100${action ? `&action=${encodeURIComponent(action)}` : ""}`, auditSchema, "گزارش ممیزی");
export const listAdminNotifications = (status: AdminNotificationStatus) =>
  list(`/admin/operations/notifications?status=${status}&per_page=100`, notificationSchema, "صف اعلان‌ها");

export async function setRoasteryStatus(idValue: string, status: AdminRoasteryStatus) {
  const raw = await apiFetch(`/admin/roasteries/${encodeURIComponent(idValue)}/status`, {
    method: "PATCH",
    body: { status },
  });
  return parseContract(resourceSchema(z.unknown()), raw, "تغییر وضعیت روستری").data;
}

export async function setProductStatus(idValue: string, status: AdminProductStatus) {
  const raw = await apiFetch(`/admin/products/${encodeURIComponent(idValue)}/status`, {
    method: "PATCH",
    body: { status },
  });
  return parseContract(resourceSchema(z.unknown()), raw, "تغییر وضعیت محصول").data;
}
...
export async function moderateReview(
  ...
  const raw = await apiFetch(`/admin/reviews/${encodeURIComponent(idValue)}`, {
    method: "PATCH",
    body: { status, reason: reason?.trim() || undefined },
  });
  return parseContract(resourceSchema(reviewSchema), raw, "مدیریت نظر").data;
}

export async function setInquiryStatus(idValue: string, status: AdminInquiryStatus) {
  const raw = await apiFetch(`/admin/inquiries/${encodeURIComponent(idValue)}`, {
    method: "PATCH",
    body: { status },
  });
  return parseContract(resourceSchema(inquirySchema), raw, "تغییر وضعیت درخواست").data;
}

export const adminRoasteriesQuery = (status: AdminRoasteryStatus) =>
  queryOptions({ queryKey: ["admin", "operations", "roasteries", status], queryFn: () => listAdminRoasteries(status), staleTime: 15_000 });
export const adminProductsQuery = (status: AdminProductStatus) =>
  queryOptions({ queryKey: ["admin", "operations", "products", status], queryFn: () => listAdminProducts(status), staleTime: 15_000 });
export const adminReviewsQuery = (status: AdminReviewStatus) =>
  queryOptions({ queryKey: ["admin", "operations", "reviews", status], queryFn: () => listAdminReviews(status), staleTime: 10_000 });
export const adminInquiriesQuery = (status: AdminInquiryStatus) =>
  queryOptions({ queryKey: ["admin", "operations", "inquiries", status], queryFn: () => listAdminInquiries(status), staleTime: 10_000 });
export const adminAuditsQuery = (action = "") =>
  queryOptions({ queryKey: ["admin", "operations", "audits", action], queryFn: () => listAdminAudits(action), staleTime: 10_000 });
export const adminNotificationsQuery = (status: AdminNotificationStatus) =>
  queryOptions({ queryKey: ["admin", "operations", "notifications", status], queryFn: () => listAdminNotifications(status), staleTime: 10_000 });
