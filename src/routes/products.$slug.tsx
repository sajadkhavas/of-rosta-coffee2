...
const PRODUCT_FAQ = [
  ...
  },
  {
    ...
    answer:
      "پس از افزودن Variant به سبد، سرور رستا قیمت و موجودی را اعتبارسنجی می‌کند و در Checkout دوباره Quote می‌سازد.",
  },
];

function productJsonLd(
  product: ProductDetail,
  reviews?: { summary: { count: number; average: number | null } },
) {
  ...
  return {
    ...
    additionalProperty: [
      ...
      { "@type": "PropertyValue", name: "سطح رست", value: roastLevelLabel(product.roastLevel) },
      {
        "@type": "PropertyValue",
        name: "فرآوری",
        value: processingLabel(product.processingMethod),
      },
  ...
  loader: async ({ params, context }) => {
    try {
      ...
      await context.queryClient
        .ensureQueryData(productReviewsQueryOptions(params.slug))
        .catch(() => undefined);
      return product;
    } catch (error) {
        ...
        <article className="mt-6 grid gap-8 md:grid-cols-2">
          <section aria-label="گالری محصول">
            <div className="aspect-square overflow-hidden rounded-2xl border border-[color:var(--mid)] bg-[color:var(--steam)]">
              {selectedImage ? (
                ...
                <div className="grid h-full place-items-center text-[color:var(--mid)]">
                  تصویر محصول
                </div>
              )}
            </div>
                ...
                {gallery.map((asset) => {
                  ...
                  return url ? (
                    <button
                      ...
                      className={`aspect-square overflow-hidden rounded-xl border ${selectedImage === url ? "border-[color:var(--roast)]" : "border-[color:var(--mid)]"}`}
                    >
                      <img
                        src={url}
          ...
          <section>
            <Link
              ...
              className="text-sm font-bold text-[color:var(--roast)]"
            >
            ...
            <div className="mt-5 flex flex-wrap gap-2 text-xs">
              <span className="rounded-full border border-[color:var(--mid)] px-3 py-1">
                {product.origin.name}
              </span>
              <span className="rounded-full border border-[color:var(--mid)] px-3 py-1">
              ...
              <span className="rounded-full border border-[color:var(--mid)] px-3 py-1">
                فرآوری {processingLabel(product.processingMethod)}
              </span>
              <span className="rounded-full border border-[color:var(--mid)] px-3 py-1">
                {product.arabicaPercentage.toLocaleString("fa-IR")}٪ عربیکا
              </span>
              {formatRoastDate(product.latestRoastBatch?.roastedAt) ? (
                <span className="rounded-full bg-[color:var(--roast)] px-3 py-1 font-bold text-[color:var(--night)]">
                  رست {formatRoastDate(product.latestRoastBatch?.roastedAt)}
                </span>
              ) : null}
            </div>
            <section className="mt-7">
              <h2 className="text-sm font-bold">انتخاب وزن دانه کامل</h2>
              <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-3">
                {product.variants.map((variant) => (
                  <button
                    ...
                    className={`rounded-xl border p-3 text-start disabled:opacity-40 ${selectedVariantId === variant.id ? "border-[color:var(--roast)] bg-[color:var(--roast)]/10" : "border-[color:var(--mid)] bg-[color:var(--dark)]"}`}
                  >
                    <span className="block text-sm font-bold">
                      {formatWeight(variant.weightGrams)}
                    </span>
                    <span className="mt-1 block text-xs text-[color:var(--light)]">
            ...
            {notice ? (
              <div className="mt-5">
                <Alert variant={noticeVariant} title="سبد خرید">
                  {notice}
                </Alert>
              </div>
            ) : null}
            <div className="mt-6 flex items-center justify-between gap-4 border-t border-[color:var(--mid)] pt-5">
              ...
              <button
                ...
                className="min-h-12 rounded-xl bg-[color:var(--roast)] px-6 text-sm font-bold text-[color:var(--night)] disabled:opacity-50"
              >
          ...
          <div className="rounded-2xl border border-[color:var(--mid)] bg-[color:var(--dark)] p-5">
            ...
            <dl className="mt-4 grid gap-3 text-sm text-[color:var(--light)]">
              <div className="flex justify-between">
                <dt>خاستگاه</dt>
                <dd>{product.origin.name}</dd>
              </div>
              <div className="flex justify-between">
                <dt>فرآوری</dt>
                <dd>{processingLabel(product.processingMethod)}</dd>
              </div>
              <div className="flex justify-between">
                <dt>رست</dt>
                <dd>{roastLevelLabel(product.roastLevel)}</dd>
              </div>
              <div className="flex justify-between">
                <dt>نت‌های چشایی</dt>
