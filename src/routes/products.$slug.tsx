...
const PRODUCT_FAQ = [
  {
    ...
    answer:
      "تمام محصولات رستا فقط به‌صورت دانه کامل ارسال می‌شوند و هیچ انتخاب آسیابی وجود ندارد.",
  },
  {
  ...
  return {
    ...
    additionalProperty: [
      ...
      },
      {
        "@type": "PropertyValue",
        name: "سطح رست",
        value: roastLevelLabel(product.roastLevel),
      },
      {
        "@type": "PropertyValue",
        name: "فرآوری",
...
export const Route = createFileRoute("/products/$slug")({
  ...
  head: ({ loaderData }) => {
    ...
    return seoHead({
      title:
        product.seo.title ||
        `${product.name} از ${product.roastery.name} | رستا`,
      description:
        product.seo.description ||
        product.shortDescription ||
        product.description,
...
function ProductPage() {
  ...
  const [noticeVariant, setNoticeVariant] = useState<"success" | "warning">(
    "success",
  );
  ...
  useEffect(() => {
    ...
    setSelectedImage(
      bestMediaUrl(product.gallery[0] ?? product.primaryImage) ?? "",
    );
  }, [product]);
  ...
  const addSelectedVariant = () => {
    ...
    if (result.status === "requires_reset") {
      ...
      setNotice(
        "تعداد Variantهای سبد به سقف امن رسیده است؛ برای افزودن محصول جدید، یک قلم را حذف کنید.",
      );
      setAdded(false);
      return;
    } else {
      setNoticeVariant("success");
      setNotice(
        "محصول اضافه شد؛ قیمت و موجودی در صفحه سبد توسط سرور تأیید می‌شود.",
      );
    }
    setAdded(true);
            ...
            <div className="mt-6 flex items-center justify-between gap-4 border-t border-[color:var(--mid)] pt-5">
              <div>
                ...
                <p className="mt-1 text-xl font-bold text-[color:var(--roast)]">
                  {selectedVariant
                    ? formatIrr(selectedVariant.price)
                    : "انتخاب وزن"}
                </p>
              </div>
              <button
          ...
          <div className="rounded-2xl border border-[color:var(--mid)] bg-[color:var(--dark)] p-5">
            ...
            <div className="mt-4 space-y-4">
              {PRODUCT_FAQ.map((item) => (
                <details
                  key={item.question}
                  className="rounded-xl border border-[color:var(--mid)] p-4 first:open"
                >
