...
function errorMessage(error: unknown) {
  return isApiError(error)
    ? error.message
    : "ثبت اطلاعات انجام نشد. قرارداد API و اتصال سرویس را بررسی کنید.";
}
  ...
  if (!roastery)
    return (
      <EmptyState
        ...
        action={
          <Link to="/panel" className="text-[color:var(--roast)] underline">
            بازگشت به پنل
          </Link>
        }
      />
      ...
      <header className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-3 max-w-3xl text-sm leading-8 text-[color:var(--light)]">
            ویرایش اطلاعات حساس روستری یا محصول منتشرشده آن را دوباره به صف بررسی ادمین می‌فرستد.
            گزینه آسیاب در هیچ بخش وجود ندارد.
          </p>
        </div>
...
function RoasteryEditor({ roastery }: { roastery: SellerRoastery }) {
  ...
  const mediaQuery = useQuery({
    queryKey: ["seller", roastery.id, "media"],
  ...
  const mutation = useMutation({
    mutationFn: () =>
      updateSellerRoastery(roastery.id, {
        name: form.name,
        slug: form.slug,
        city: form.city,
  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-xl font-bold">اطلاعات روستری</h2>
          <p className="mt-1 text-xs text-[color:var(--light)]">وضعیت فعلی: {roastery.status}</p>
        </div>
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-2">
        ...
        <div className="grid grid-cols-2 gap-3">
          <TextField
            ...
            onChange={(event) =>
              setForm((current) => ({
                ...current,
                minHours: event.target.value.replace(/\D/g, ""),
              }))
            }
          />
          <TextField
            ...
            onChange={(event) =>
              setForm((current) => ({
                ...current,
                maxHours: event.target.value.replace(/\D/g, ""),
              }))
            }
          />
        </div>
        ...
        <div className="md:col-span-2">
          <TextareaField
            ...
            onChange={(event) =>
              setForm((current) => ({ ...current, shippingPolicy: event.target.value }))
            }
          />
        </div>
        <MediaSelect
          ...
          media={media}
          onChange={(value) => setForm((current) => ({ ...current, logoMediaId: value }))}
        />
        <MediaSelect
          label="کاور"
          value={form.coverMediaId}
          media={media}
          onChange={(value) => setForm((current) => ({ ...current, coverMediaId: value }))}
        />
      </div>
      {mutation.isError ? (
        <div className="mt-4">
...
function ProductEditor({ roastery }: { roastery: SellerRoastery }) {
  ...
  useEffect(() => {
    if (!productId && products.data?.items.length) setProductId(products.data.items[0].id);
  }, [productId, products.data]);
  const detail = useQuery({
    queryKey: ["seller", roastery.id, "products", productId, "detail"],
    queryFn: () => getSellerProduct(roastery.id, productId),
  ...
  const mutation = useMutation({
    mutationFn: () =>
      updateSellerProduct(roastery.id, productId, {
        ...
        brewingSuggestions: form.brewingSuggestions
          .split(/\n+/)
          .map((item) => item.trim())
          .filter(Boolean),
  ...
  return (
    <section className="rounded-2xl border border-[color:var(--mid)] bg-[color:var(--dark)] p-5">
      <header className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h2 className="text-xl font-bold">ویرایش محصول، SEO و رسانه</h2>
          <p className="mt-1 text-xs text-[color:var(--light)]">
            محصول منتشرشده با ویرایش محتوا دوباره Review می‌شود.
          </p>
        </div>
        <select
          value={productId}
      ...
      {products.isLoading || detail.isLoading ? (
        <Skeleton className="mt-5 h-96" />
      ) : products.isError || detail.isError ? (
        <div className="mt-5">
          <Alert variant="danger">{errorMessage(products.error || detail.error)}</Alert>
        </div>
      ) : !product ? (
        <div className="mt-5">
          <EmptyState title="محصولی انتخاب نشده" />
        </div>
      ) : (
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <label className="grid gap-2 text-sm font-bold">
              ...
              <select
                value={form.originId}
                onChange={(event) =>
                  setForm((current) => ({ ...current, originId: event.target.value }))
                }
                className={fieldClass}
              >
            ...
            <TextField
              ...
              onChange={(event) =>
                setForm((current) => ({
                  ...current,
                  arabicaPercentage: event.target.value.replace(/\D/g, "").slice(0, 3),
                }))
              }
            />
            <label className="grid gap-2 text-sm font-bold">
            ...
            <div className="md:col-span-2">
              <TextField
                label="یادداشت‌های طعمی"
                value={form.tastingNotes}
                onChange={(event) =>
                  setForm((current) => ({ ...current, tastingNotes: event.target.value }))
                }
              />
            </div>
            <div className="md:col-span-2">
            ...
            <div className="md:col-span-2">
              <TextareaField
                label="پیشنهادهای دم‌آوری؛ هر مورد یک خط"
                value={form.brewingSuggestions}
                onChange={(event) =>
                  setForm((current) => ({ ...current, brewingSuggestions: event.target.value }))
                }
              />
            </div>
            <TextField
            ...
            <MediaSelect
              ...
              onChange={(value) => setForm((current) => ({ ...current, primaryMediaId: value }))}
            />
            <GallerySelect
              media={media.data?.items ?? []}
              selected={form.galleryMediaIds}
              onChange={(galleryMediaIds) =>
                setForm((current) => ({ ...current, galleryMediaIds }))
              }
            />
          </div>
...
function VariantEditors({
  ...
  const [drafts, setDrafts] = useState<
    Record<string, { sku: string; price: string; compareAtPrice: string; isActive: boolean }>
  >({});
  useEffect(
    () =>
      setDrafts(
  ...
  const mutation = useMutation({
    mutationFn: ({
      ...
      id: string;
      value: { sku: string; price: string; compareAtPrice: string; isActive: boolean };
    }) =>
      updateSellerVariant(roasteryId, productId, id, {
        sku: value.sku,
  ...
  return (
    <div className="border-t border-[color:var(--mid)] pt-5">
      <h3 className="font-bold">ویرایش Variantهای دانه کامل</h3>
      <div className="mt-4 grid gap-3">
        {variants.map((variant) => {
          const value = drafts[variant.id];
          if (!value) return null;
          return (
            <div
              key={variant.id}
              className="grid gap-3 rounded-xl border border-[color:var(--mid)] bg-[color:var(--night)] p-4 md:grid-cols-[auto_1fr_1fr_1fr_auto]"
            >
              <span className="self-center text-sm font-bold">{toFa(variant.weightGrams)} گرم</span>
              <input
                dir="ltr"
                value={value.sku}
                onChange={(event) =>
                  setDrafts((current) => ({
                    ...current,
                    [variant.id]: { ...value, sku: event.target.value },
                  }))
                }
                className={fieldClass}
              />
              <input
                ...
                onChange={(event) =>
                  setDrafts((current) => ({
                    ...current,
                    [variant.id]: { ...value, price: event.target.value.replace(/\D/g, "") },
                  }))
                }
                className={fieldClass}
              />
              <input
                ...
                onChange={(event) =>
                  setDrafts((current) => ({
                    ...
                    [variant.id]: {
                      ...value,
                      compareAtPrice: event.target.value.replace(/\D/g, ""),
                    },
                  }))
                }
                className={fieldClass}
              ...
              <Button
                ...
                onClick={() => mutation.mutate({ id: variant.id, value })}
              >
                ذخیره
              </Button>
            </div>
...
function MediaSelect({
  ...
  label: string;
  value: string;
  media: Array<{ id: string; alt: string; sources: Array<{ url: string }> }>;
  ...
  return (
    <label className="grid gap-2 text-sm font-bold">
      ...
      {value ? (
        <img
          src={mediaUrl(media.find((item) => item.id === value)) || ""}
          alt="پیش‌نمایش"
...
function GallerySelect({
  ...
  return (
    <fieldset className="rounded-xl border border-[color:var(--mid)] p-3">
      <legend className="px-2 text-sm font-bold">Gallery</legend>
