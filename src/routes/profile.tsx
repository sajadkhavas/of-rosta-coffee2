...
function ProfileContent({ user }: { user: AuthUser }) {
  ...
  const saveAddressMutation = useMutation({
    mutationFn: async () =>
      editingAddress ? updateAddress(editingAddress.id, addressForm) : createAddress(addressForm),
    onSuccess: async () => {
      ...
      pushToast({
        variant: "success",
        title: editingAddress ? "آدرس ویرایش شد" : "آدرس افزوده شد",
      });
    },
  });
  ...
  const submitAddress = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!addressForm.recipientName.trim())
      errors.push({ fieldId: "address-recipient", message: "نام گیرنده را وارد کنید." });
    if (!/^09\d{9}$/.test(addressForm.recipientMobile.replace(/\s/g, "")))
      errors.push({ fieldId: "address-mobile", message: "شماره موبایل گیرنده معتبر نیست." });
    if (!addressForm.province.trim())
      errors.push({ fieldId: "address-province", message: "استان را وارد کنید." });
    if (!addressForm.city.trim())
      errors.push({ fieldId: "address-city", message: "شهر را وارد کنید." });
    if (addressForm.addressLine.trim().length < 10)
      errors.push({ fieldId: "address-line", message: "نشانی کامل‌تر وارد کنید." });
  ...
  return (
    <>
      <header className="mt-4 flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-sm text-[color:var(--light)]">
            نشست امن مبتنی بر Cookie؛ بدون ذخیره Token در مرورگر.
          </p>
        </div>
      ...
      <div className="mt-8 grid gap-5 md:grid-cols-2">
        <section className="rounded-2xl border border-[color:var(--mid)] bg-[color:var(--dark)] p-5">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p dir="ltr" className="mt-1 text-start font-mono text-xs text-[color:var(--light)]">
                {user.mobile}
              </p>
              {user.email ? (
                <p dir="ltr" className="mt-1 text-start text-xs text-[color:var(--light)]">
                  {user.email}
                </p>
              ) : null}
            </div>
          ...
          {tasteProfile ? (
            <>
              <ul className="mt-3 space-y-1 text-sm leading-7 text-[color:var(--light)]">
                ...
                {tasteProfile.flavors.length ? (
                  <li>طعم‌ها: {tasteProfile.flavors.join("، ")}</li>
                ) : null}
              </ul>
              <p className="mt-3 text-[11px] leading-6 text-[color:var(--light)]">
                این نتیجه فعلاً فقط برای UX محلی کوییز نگه‌داری می‌شود؛ همگام‌سازی Backend در فاز
                کوییز انجام خواهد شد.
              </p>
              <Link
                to="/quiz"
                className="mt-3 inline-block text-xs font-bold text-[color:var(--roast)] underline"
              >
                ویرایش سلیقه
              </Link>
            </>
            ...
            <EmptyState
              ...
              action={
                <Link
                  to="/quiz"
                  className="inline-flex rounded-xl bg-[color:var(--roast)] px-4 py-2 text-xs font-bold text-[color:var(--night)]"
                >
                  شروع کوییز
                </Link>
              }
        ...
        <section className="rounded-2xl border border-[color:var(--mid)] bg-[color:var(--dark)] p-5 md:col-span-2">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-xs text-[color:var(--light)]">
                آدرس‌ها از حساب واقعی API دریافت می‌شوند.
              </p>
            </div>
          ...
          {addressesQuery.isPending ? (
            ...
            <div className="mt-5">
              ...
              <Button variant="outline" className="mt-3" onClick={() => addressesQuery.refetch()}>
                تلاش مجدد
              </Button>
            </div>
          ) : addresses.length === 0 ? (
            <div className="mt-5">
              <EmptyState
                ...
                description="برای Checkout سریع‌تر، نشانی گیرنده را ذخیره کنید."
              />
            </div>
          ) : (
              ...
              {addresses.map((address) => (
                ...
                >
                  <div className="flex items-center justify-between gap-3">
                    ...
                    {address.isDefault ? (
                      <span className="rounded-full bg-[color:var(--roast)] px-2 py-0.5 text-[10px] font-bold text-[color:var(--night)]">
                        پیش‌فرض
                      </span>
                    ) : null}
                  </div>
                  ...
                  <p className="mt-2 text-xs text-[color:var(--light)]">
                    گیرنده: {address.recipientName} · {address.recipientMobile}
                  </p>
        ...
        <nav
          className="rounded-2xl border border-[color:var(--mid)] bg-[color:var(--dark)] p-5 md:col-span-2"
          aria-label="دسترسی‌های حساب"
        >
          <ul className="divide-y divide-[color:var(--mid)] text-sm">
            <li>
              <Link
                to="/orders"
                className="flex items-center justify-between py-4 font-bold hover:text-[color:var(--roast)]"
              >
            ...
            <li>
              <button
                ...
                className="w-full py-4 text-start font-bold text-red-300 disabled:opacity-50"
              >
      ...
      <Dialog
        ...
        title="ویرایش مشخصات"
        description="شماره موبایل از مسیر تأیید جداگانه تغییر می‌کند."
      >
        <form onSubmit={submitProfile} className="grid gap-4" noValidate>
          <FormSummary errors={profileErrors} />
          {profileMutation.isError ? (
            <Alert variant="danger">
              {isApiError(profileMutation.error)
                ? profileMutation.error.message
                : "ذخیره مشخصات انجام نشد."}
            </Alert>
          ) : null}
          <TextField
            id="profile-name"
          ...
          <TextField
            ...
            onChange={(event) =>
              setProfileForm((value) => ({ ...value, email: event.target.value }))
            }
          />
      ...
      <Dialog
        ...
        title={editingAddress ? "ویرایش آدرس" : "افزودن آدرس"}
        description="اطلاعات گیرنده برای سفارش و ارسال استفاده می‌شود."
      >
        <form onSubmit={submitAddress} className="grid gap-4" noValidate>
          <FormSummary errors={addressErrors} />
          {saveAddressMutation.isError ? (
            <Alert variant="danger">
              {isApiError(saveAddressMutation.error)
                ? saveAddressMutation.error.message
                : "ذخیره آدرس انجام نشد."}
            </Alert>
          ) : null}
          <TextField
            label="عنوان آدرس"
            ...
            onChange={(event) =>
              setAddressForm((value) => ({ ...value, title: event.target.value }))
            }
          />
          <TextField
            ...
            onChange={(event) =>
              setAddressForm((value) => ({ ...value, recipientName: event.target.value }))
            }
            required
          />
          <TextField
            ...
            onChange={(event) =>
              setAddressForm((value) => ({ ...value, recipientMobile: event.target.value }))
            }
            required
          />
          <div className="grid gap-4 sm:grid-cols-2">
            <TextField
              ...
              onChange={(event) =>
                setAddressForm((value) => ({ ...value, province: event.target.value }))
              }
              required
            />
            <TextField
              ...
              onChange={(event) =>
                setAddressForm((value) => ({ ...value, city: event.target.value }))
              }
              required
            />
          </div>
          <TextareaField
            ...
            onChange={(event) =>
              setAddressForm((value) => ({ ...value, addressLine: event.target.value }))
            }
            required
          />
          <TextField
            ...
            onChange={(event) =>
              setAddressForm((value) => ({ ...value, postalCode: event.target.value }))
            }
          />
          <label className="flex items-center gap-3 text-sm">
            <input
              ...
              onChange={(event) =>
                setAddressForm((value) => ({ ...value, isDefault: event.target.checked }))
              }
          ...
          <Button type="submit" loading={saveAddressMutation.isPending}>
            {editingAddress ? "ذخیره ویرایش" : "افزودن آدرس"}
          </Button>
        </form>
      </Dialog>
    </>
