...
export const Route = createFileRoute("/panel/settlement-profile")({
  head: () => ({
    meta: [{ title: "مقصد تسویه روستری | رستا" }, { name: "robots", content: "noindex,nofollow" }],
  }),
  ...
  if (roasteries.isError) {
    return (
      <Alert variant="danger">
        {isApiError(roasteries.error) ? roasteries.error.message : "روستری‌ها دریافت نشدند."}
      </Alert>
    );
  }
...
function ProfileEditor({ roasteryId, canEdit }: { roasteryId: string; canEdit: boolean }) {
  ...
  if (query.isError) {
    return (
      <Alert variant="danger">
        {isApiError(query.error) ? query.error.message : "پروفایل تسویه دریافت نشد."}
      </Alert>
    );
  }
  ...
  return (
    ...
    >
      ...
      {profile?.status === "verified" ? (
        <div className="mt-4">
          <Alert variant="success">این مقصد برای ساخت Batch تسویه معتبر است.</Alert>
        </div>
      ) : null}
      {profile?.status === "rejected" ? (
        <div className="mt-4">
          <Alert variant="warning" title="نیاز به اصلاح">
            {profile.review_note || "اطلاعات را اصلاح و دوباره ارسال کنید."}
          </Alert>
        </div>
      ) : null}
      {!canEdit ? (
        <div className="mt-4">
          <Alert variant="info">
            نقش شما فقط اجازه مشاهده اطلاعات مالی را دارد؛ تغییر مقصد فقط برای مالک یا مدیر مجاز
            است.
          </Alert>
        </div>
      ) : null}
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-2">
        <label className="text-xs font-bold text-[color:var(--light)]">
          ...
          <select
            value={entityType}
            disabled={!canEdit}
            onChange={(event) => setEntityType(event.target.value as SettlementEntityType)}
      ...
      <p className="mt-4 text-xs leading-6 text-[color:var(--light)]">
        شماره شبا پس از ثبت به‌صورت encrypted در Backend نگه‌داری می‌شود و در پنل فروشنده فقط نسخه
        ماسک‌شده برمی‌گردد.
      </p>
      ...
      {canEdit ? (
        <Button
          ...
          disabled={
            ...
            !/^IR\d{24}$/.test(iban.replace(/\s+/g, "").toUpperCase())
          }
        >
      ...
      {mutation.isSuccess ? (
        <div className="mt-4">
          <Alert variant="success">پروفایل برای بررسی ادمین ارسال شد.</Alert>
        </div>
      ) : null}
      {mutation.isError ? (
        <div className="mt-4">
          <Alert variant="danger">
            {isApiError(mutation.error) ? mutation.error.message : "ثبت پروفایل انجام نشد."}
          </Alert>
        </div>
      ) : null}
    </form>
