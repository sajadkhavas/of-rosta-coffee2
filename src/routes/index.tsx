    ...
    return {
      meta: [
        { title: "رستا | خرید قهوه تازه مستقیم از روستری" },
        {
          name: "description",
          content:
            "رستا مارکت‌پلیس دانه کامل قهوه تازه‌رست از روستری‌های تأییدشده ایران؛ با موجودی، قیمت و تاریخ رست زنده.",
        },
        { property: "og:title", content: "رستا | خرید قهوه تازه مستقیم از روستری" },
        {
          property: "og:description",
          content: "دانه کامل قهوه تازه‌رست با اطلاعات زنده کاتالوگ و روستری.",
        },
      ...
      <main>
          ...
          <div className="relative mx-auto grid max-w-6xl gap-10 px-4 py-20 md:min-h-screen md:grid-cols-2 md:items-center md:py-28">
            <div className="text-center md:text-right">
              ...
              <div className="mx-auto mt-10 flex flex-wrap items-center justify-center gap-6 md:mx-0 md:justify-start">
                ...
                <Link to="/roasteries" className="btn-ghost">
                  کشف روستری‌ها <span aria-hidden>←</span>
                </Link>
              </div>
              <dl className="mt-14 grid grid-cols-3 gap-4 border-t border-[color:var(--mid)] pt-8">
                {[
                  ...
                  <div key={stat.label} className="text-center md:text-right">
                    ...
                    <dd className="mt-2 text-[11px] tracking-[0.1em] text-[color:var(--muted-gold)]">
                      {stat.label}
                    </dd>
                  </div>
                ))}
        ...
        >
          <ul className="mx-auto grid max-w-6xl grid-cols-2 gap-3 px-4 py-6 text-sm md:grid-cols-4">
            {["تازه‌رست", "اطلاعات زنده", "فقط دانه کامل", "خرید مستقیم"].map((item) => (
              <li key={item} className="flex items-center gap-3 text-[color:var(--light)]">
        ...
        <section className="mx-auto max-w-6xl px-4 py-20">
          <Header
            eyebrow="روستری‌های زنده"
            title="روستری‌های تأییدشده"
            link="/roasteries"
            linkLabel="مشاهده همه"
          />
          {data.roasteries.length ? (
            <div className="grid gap-5 md:grid-cols-3">
              {data.roasteries.map((roastery) => (
                <HomeRoasteryCard key={roastery.id} roastery={roastery} />
              ))}
            </div>
          ) : (
            <EmptyState
              title="روستری منتشرشده‌ای وجود ندارد"
              description="پس از تأیید نخستین روستری، این بخش به‌صورت خودکار به‌روزرسانی می‌شود."
            />
          )}
        </section>
        ...
        <section className="border-y border-[color:var(--mid)] bg-[color:var(--dark)]">
          <div className="mx-auto max-w-6xl px-4 py-20">
            <div className="text-center">
              ...
              <h2 className="mt-3 font-display text-3xl font-bold text-[color:var(--steam)] md:text-5xl">
                از روستری تا فنجان
              </h2>
            </div>
            <ol className="mt-12 grid gap-6 md:grid-cols-3">
              {[
                [
                  "۰۱",
                  "روستری و محصول را انتخاب کن",
                  "فقط موارد تأییدشده و منتشرشده نمایش داده می‌شوند.",
                ],
                ["۰۲", "وزن دانه کامل را انتخاب کن", "وزن‌های ثابت ۵۰، ۱۰۰، ۲۵۰، ۵۰۰ و ۱۰۰۰ گرم."],
                [
                  "۰۳",
                  "وضعیت سفارش را زنده پیگیری کن",
                  "پذیرش، آماده‌سازی، ارسال و تحویل از Backend معتبر.",
                ],
              ].map(([number, title, description]) => (
                <li key={number} className="card-dark rounded-2xl p-7">
                  <span className="font-mono-num text-[color:var(--roast)]">{number}</span>
                  <h3 className="mt-4 text-lg font-bold">{title}</h3>
                  <p className="mt-3 text-sm leading-7 text-[color:var(--light)]">{description}</p>
                </li>
              ))}
            </ol>
          </div>
        ...
        <section className="mx-auto max-w-6xl px-4 py-20">
          ...
          {data.products.length ? (
            ...
            <EmptyState
              title="محصول منتشرشده‌ای وجود ندارد"
              description="رستا در Production هیچ محصول Seed یا ساختگی نمایش نمی‌دهد."
            />
          )}
        </section>
        ...
        <section className="mx-auto max-w-6xl px-4 pb-20">
          <div className="rounded-3xl border border-[color:var(--mid)] bg-gradient-to-br from-[#1a0a00] via-[#2a1405] to-[#0a0400] px-8 py-14 text-center">
            ...
            <h2 className="mt-4 font-display text-3xl font-bold text-[color:var(--steam)] md:text-5xl">
              قهوه مناسب ذائقه‌ات را پیدا کن
            </h2>
            <p className="mx-auto mt-4 max-w-xl text-sm leading-7 text-[color:var(--light)]">
              کوییز فقط بین محصولات منتشرشده و موجود API جست‌وجو می‌کند.
            </p>
            <Link
              to="/quiz"
              className="mt-7 inline-flex rounded-xl bg-[color:var(--roast)] px-8 py-3 text-sm font-bold text-[color:var(--night)]"
            >
              شروع کوییز
            </Link>
          </div>
          ...
          <section className="border-t border-[color:var(--mid)] bg-[color:var(--dark)]">
            <div className="mx-auto max-w-3xl px-4 py-20">
              <div className="text-center">
                ...
                <h2 className="mt-3 font-display text-3xl font-bold text-[color:var(--steam)]">
                  پرسش‌های پرتکرار
                </h2>
              </div>
              <div className="mt-10 space-y-3">
                {data.faqs.map((faq) => (
                  <details
                    key={faq.question}
                    className="rounded-xl border border-[color:var(--mid)] bg-[color:var(--night)] p-5"
                  >
                    <summary className="cursor-pointer font-medium">{faq.question}</summary>
                    <p className="mt-3 text-sm leading-7 text-[color:var(--light)]">{faq.answer}</p>
                  </details>
                ))}
              </div>
            </div>
...
function Header({
  ...
  return (
    <div className="mb-10 flex items-end justify-between gap-4">
      ...
      <Link to={link} className="text-sm font-bold text-[color:var(--roast)]">
        {linkLabel} ←
      </Link>
    </div>
  );
