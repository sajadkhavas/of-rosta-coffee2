...
function orderStage(status: OrderStatus, subStatuses: SubOrderStatus[]) {
  ...
  if (
    status === "processing" ||
    subStatuses.some((value) => ["preparing", "ready_to_ship"].includes(value))
  )
    return 2;
  if (
    status === "paid" ||
    subStatuses.some((value) =>
      ["accepted", "preparing", "ready_to_ship", "shipped", "delivered"].includes(value),
    )
  )
    return 1;
  return 0;
...
function OrderContent() {
    ...
    return (
      <section className="mx-auto max-w-xl py-12 text-center">
        <h1 className="text-2xl font-bold">
          {missing ? "سفارش پیدا نشد" : "جزئیات سفارش دریافت نشد"}
        </h1>
        ...
        <div className="mt-6 flex justify-center gap-3">
          {!missing ? <Button onClick={() => query.refetch()}>تلاش مجدد</Button> : null}
          <Link
            to="/orders"
  ...
  const stage = orderStage(
    order.status,
    order.subOrders.map((item) => item.status),
  );
  ...
  return (
    <>
      ...
      />
      <header className="mt-5 rounded-2xl border border-[color:var(--mid)] bg-[color:var(--dark)] p-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="text-xs font-bold text-[color:var(--roast)]">ORDER</p>
      ...
      {!["cancelled", "partially_cancelled", "refunded"].includes(order.status) ? (
        <section className="mt-5 rounded-2xl border border-[color:var(--mid)] bg-[color:var(--dark)] p-5">
          <h2 className="mb-6 font-bold">وضعیت کلی</h2>
          <ol className="grid gap-3 md:grid-cols-5">
            {timelineSteps.map((label, index) => (
              <li key={label} className="flex items-center gap-3 md:flex-col">
                <span
                  className={`grid size-9 place-items-center rounded-full border-2 ${index <= stage ? "border-[color:var(--roast)] bg-[color:var(--roast)] text-[color:var(--night)]" : "border-[color:var(--mid)]"}`}
                >
                  {index < stage ? <Check size={15} /> : (index + 1).toLocaleString("fa-IR")}
                </span>
                <span
                  className={`text-xs ${index === stage ? "font-bold text-[color:var(--roast)]" : "text-[color:var(--light)]"}`}
                >
                  {label}
                </span>
              </li>
        ...
        <div className="mt-5">
          <Alert variant={order.status === "refunded" ? "info" : "warning"}>
            {orderStatusLabels[order.status]}
          </Alert>
        </div>
      )}
      ...
      <div className="mt-5 grid gap-5 lg:grid-cols-[1fr_19rem]">
            ...
            >
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div>
                  ...
                  <p className="mt-1 text-xs text-[color:var(--light)]">زیرسفارش #{subOrder.id}</p>
                </div>
                <span
                    ...
                    <div className="flex gap-4">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="mt-1 text-xs text-[color:var(--light)]">
                          {formatWeight(item.variant.weightGrams)} · دانه کامل ·{" "}
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-4">
                    <div>
                      <p className="text-xs text-[color:var(--light)]">
                        ارسال{" "}
                        {subOrder.shipment.carrier ? `با ${subOrder.shipment.carrier}` : "سفارش"}
                      </p>
                      {subOrder.shipment.trackingCode ? (
                        <p
                          dir="ltr"
                          className="mt-1 font-mono text-sm font-bold text-[color:var(--roast)]"
                        >
                          {subOrder.shipment.trackingCode}
                        </p>
                      ) : null}
              ...
              <dl className="mt-5 space-y-2 border-t border-[color:var(--mid)] pt-4 text-sm">
                ...
                <div className="flex justify-between text-[color:var(--light)]">
                  <dt>بسته‌بندی</dt>
                  <dd>
                    {subOrder.packagingTotal === 0 ? "رایگان" : formatIrr(subOrder.packagingTotal)}
                  </dd>
                </div>
                <div className="flex justify-between text-[color:var(--light)]">
                  <dt>آسیاب</dt>
        ...
        <aside className="h-fit rounded-2xl border border-[color:var(--mid)] bg-[color:var(--dark)] p-5 lg:sticky lg:top-24">
          ...
          <dl className="mt-4 space-y-3 text-sm">
            ...
            <div className="flex justify-between border-t border-[color:var(--mid)] pt-3 font-bold">
              <dt>مبلغ نهایی</dt>
              <dd className="text-[color:var(--roast)]">{formatIrr(order.grandTotal)}</dd>
            </div>
          </dl>
          {canReview ? (
            <p className="mt-5 text-xs leading-6 text-[color:var(--muted-gold)]">
              برای هر آیتم سفارش تحویل‌شده یک نظر خرید تأییدشده می‌توان ثبت کرد. نظر ابتدا Pending
              است.
            </p>
          ) : null}
        </aside>
  ...
  const mutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      await queryClient.invalidateQueries({
        queryKey: ["public", "products", item.product.slug, "reviews"],
      });
    },
  });
  const submit = (event: FormEvent) => {
    event.preventDefault();
    if (body.trim().length >= 10) mutation.mutate();
  };
  if (mutation.isSuccess)
    return (
      <Alert variant="success" title="نظر ثبت شد">
        نظر خرید تأییدشده در انتظار Moderation است.
      </Alert>
    );
  return (
        ...
        <form onSubmit={submit} className="space-y-3">
          <label className="grid gap-2 text-xs font-bold">
            ...
            <select
              ...
              className="min-h-10 rounded-xl border border-[color:var(--mid)] bg-[color:var(--night)] px-3"
            >
              {[5, 4, 3, 2, 1].map((value) => (
          ...
          <input
            ...
            className="min-h-10 w-full rounded-xl border border-[color:var(--mid)] bg-[color:var(--night)] px-3 text-sm"
          />
          <textarea
            required
            minLength={10}
            ...
            placeholder="تجربه واقعی خود را بنویسید"
            className="min-h-28 w-full rounded-xl border border-[color:var(--mid)] bg-[color:var(--night)] p-3 text-sm"
          />
          {mutation.isError ? (
            <Alert variant="danger">
              {isApiError(mutation.error) ? mutation.error.message : "ثبت نظر انجام نشد."}
            </Alert>
          ) : null}
