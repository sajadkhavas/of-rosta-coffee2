  ...
  return (
    <>
      ...
      <div className="mt-5 grid gap-5 lg:grid-cols-[1fr_19rem]">
        ...
        <aside className="h-fit rounded-2xl border border-[color:var(--mid)] bg-[color:var(--dark)] p-5 lg:sticky lg:top-24"><h2 className="font-bold">خلاصه پرداخت</h2><dl className="mt-4 space-y-3 text-sm"><div className="flex justify-between text-[color:var(--light)]"><dt>جمع محصولات</dt><dd>{formatIrr(order.subtotal)}</dd></div><div className="flex justify-between text-[color:var(--light)]"><dt>ارسال</dt><dd>{formatIrr(order.shippingTotal)}</dd></div><div className="flex justify-between text-[color:var(--light)]"><dt>تخفیف</dt><dd>- {formatIrr(order.discountTotal)}</dd></div><div className="flex justify-between border-t border-[color:var(--mid)] pt-3 font-bold"><dt>مبلغ نهایی</dt><dd className="text-[color:var(--roast)]">{formatIrr(order.grandTotal)}</dd></div></dl>{canReview ? <p className="mt-5 text-xs leading-6 text-[color:var(--muted-gold)]">برای هر آیتم سفارش تحویل‌شده یک نظر خرید تأییدشده می‌توان ثبت کرد. نظر ابتدا Pending است.</p> : null}</aside>
      </div>
    </>
  );
}
