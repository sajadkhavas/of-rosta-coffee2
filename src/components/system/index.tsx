...
const buttonVariants: Record<ButtonVariant, string> = {
  primary:
    "border-transparent bg-[color:var(--roast)] text-[color:var(--night)] hover:brightness-110",
  secondary:
    "border-[color:var(--mid)] bg-[color:var(--dark)] text-[color:var(--steam)] hover:border-[color:var(--roast)]",
  outline:
    "border-[color:var(--roast)] bg-transparent text-[color:var(--roast)] hover:bg-[color:var(--roast)] hover:text-[color:var(--night)]",
  ghost:
    "border-transparent bg-transparent text-[color:var(--light)] hover:bg-white/5 hover:text-[color:var(--steam)]",
...
export function Container({ children, className }: { children: ReactNode; className?: string }) {
  return (
    <div className={classes("mx-auto w-full max-w-6xl px-4 sm:px-6", className)}>{children}</div>
  );
}
...
export function PageHeader({
  ...
  return (
    <header className="grid gap-5 md:grid-cols-[1fr_auto] md:items-end">
      <div>
        {eyebrow ? (
          <p className="text-xs font-bold tracking-[0.2em] text-[color:var(--roast)]">{eyebrow}</p>
        ) : null}
        <h1 className="mt-2 text-3xl font-bold text-[color:var(--steam)] sm:text-4xl">{title}</h1>
        {description ? (
          <div className="mt-3 max-w-2xl text-sm leading-7 text-[color:var(--light)]">
            {description}
          </div>
        ) : null}
      </div>
...
export function TextField({
  ...
  return (
    <label htmlFor={id} className="grid gap-2 text-sm font-bold text-[color:var(--steam)]">
      ...
      />
      {error ? (
        <span id={`${id}-error`} role="alert" className="text-xs font-normal text-red-300">
          {error}
        </span>
      ) : null}
      {!error && description ? (
        <span
          id={`${id}-description`}
          className="text-xs font-normal leading-6 text-[color:var(--light)]"
        >
          {description}
        </span>
      ) : null}
  ...
  return (
    <label htmlFor={id} className="grid gap-2 text-sm font-bold text-[color:var(--steam)]">
      ...
      <textarea
        ...
        aria-invalid={Boolean(error)}
        className={classes(
          "min-h-32 resize-y rounded-xl border border-[color:var(--mid)] bg-[color:var(--night)] p-4 text-sm font-normal outline-none focus:border-[color:var(--roast)]",
          error && "border-red-500",
          className,
        )}
        {...props}
      ...
      {!error && description ? (
        <span className="text-xs font-normal text-[color:var(--light)]">{description}</span>
      ) : null}
    </label>
  );
}
  ...
  return (
    <label className="flex cursor-pointer items-start gap-3 rounded-xl border border-[color:var(--mid)] bg-[color:var(--night)] p-3 text-sm">
      ...
      <span>
        ...
        {description ? (
          <span className="mt-1 block text-xs leading-6 text-[color:var(--light)]">
            {description}
          </span>
        ) : null}
      </span>
...
export function Alert({
  ...
  return (
    <div
      ...
      className={classes(
        "rounded-2xl border p-4 text-sm leading-7 text-[color:var(--light)]",
        styles[variant],
      )}
    >
      {title ? <p className="mb-1 font-bold text-[color:var(--steam)]">{title}</p> : null}
      {children}
    </div>
  );
}
...
export function EmptyState({
  ...
  return (
    <div className="rounded-2xl border border-dashed border-[color:var(--mid)] p-10 text-center">
      <h2 className="font-bold">{title}</h2>
      {description ? (
        <p className="mt-2 text-sm leading-7 text-[color:var(--light)]">{description}</p>
      ) : null}
...
export function Skeleton({ className }: { className?: string }) {
  return (
    <div
      aria-hidden
      className={classes("animate-pulse rounded-xl bg-[color:var(--dark)]", className)}
    />
  );
}
...
export function FormSummary({ errors }: { errors: readonly FormSummaryError[] }) {
  ...
  return (
    <div role="alert" className="rounded-2xl border border-red-400/40 bg-red-950/20 p-4">
      <p className="font-bold">خطاهای فرم را بررسی کنید</p>
      <ul className="mt-2 list-disc pe-5 text-sm text-red-200">
        {errors.map((error) => (
    ...
    <dialog
      ...
      className={classes(
        ...
        variant === "dialog"
          ...
          : "me-0 h-dvh max-h-dvh max-w-md rounded-none",
      )}
    >
      <header className="flex items-start justify-between gap-4 border-b border-[color:var(--mid)] p-5">
        <div>
          <h2 className="text-xl font-bold">{title}</h2>
          {description ? (
            <p className="mt-2 text-sm text-[color:var(--light)]">{description}</p>
          ) : null}
        </div>
        <button
          type="button"
...
export function Dialog(props: Omit<Parameters<typeof Overlay>[0], "variant">) {
  return <Overlay {...props} variant="dialog" />;
}
export function Drawer(props: Omit<Parameters<typeof Overlay>[0], "variant">) {
...
export function ToastProvider({ children }: { children: ReactNode }) {
  ...
  return (
    <ToastContext.Provider value={value}>
      ...
      <div
        aria-live="polite"
        className="fixed bottom-20 start-4 z-[100] grid w-[min(24rem,calc(100%-2rem))] gap-3 md:bottom-4"
      >
        {toasts.map((toast) => (
          <div
            key={toast.id}
            className="rounded-2xl border border-[color:var(--mid)] bg-[color:var(--dark)] p-4 shadow-2xl"
          >
