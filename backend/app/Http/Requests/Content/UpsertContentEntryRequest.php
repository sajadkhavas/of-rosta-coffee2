...
final class UpsertContentEntryRequest extends FormRequest
{
    ...
    protected function prepareForValidation(): void
    {
        ...
        if (is_array($relations)) {
            $this->rejectUnexpectedNested(
                $relations,
                ['relation_type', 'target_type', 'target_key', 'anchor_text', 'position'],
                'relations',
            );
        }
    }
    ...
    public function rules(): array
    {
        ...
        return [
            ...
            'canonical_path' => [$required, 'string', 'min:1', 'max:500', Rule::unique('content_entries', 'canonical_path')->ignore($entryId)],
            ...
            'schema_type' => ['sometimes', Rule::in(['Article', 'BlogPosting', 'CollectionPage', 'FAQPage', 'WebPage'])],
            'keywords' => ['sometimes', 'array', 'max:30'],
            'keywords.*' => ['string', 'min:1', 'max:120'],
            'relations' => ['sometimes', 'array', 'max:100'],
            'relations.*.relation_type' => ['required', Rule::in(['related', 'mentions', 'recommends', 'compares', 'primary_topic'])],
            'relations.*.target_type' => ['required', Rule::in(['content', 'product', 'roastery', 'origin', 'brew_method', 'taste'])],
            'relations.*.target_key' => ['required', 'string', 'min:1', 'max:240'],
            'relations.*.anchor_text' => ['nullable', 'string', 'max:300'],
            'relations.*.position' => ['sometimes', 'integer', 'min:0', 'max:1000'],
        ];
    }
}
