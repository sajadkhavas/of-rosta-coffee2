...
final class UpsertProductRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        return [
            ...
            'primary_media_id' => ['nullable', 'string', 'max:200'],
            'name' => [$required, 'string', 'min:1', 'max:240'],
            'slug' => [$required, 'string', 'min:1', 'max:180', 'regex:/^[A-Za-z0-9\x{0600}-\x{06FF}](?:[A-Za-z0-9\x{0600}-\x{06FF}_-]{0,178}[A-Za-z0-9\x{0600}-\x{06FF}])?$/u', Rule::unique('products', 'slug')->ignore($productId)],
            'short_description' => ['nullable', 'string', 'max:1000'],
            'description' => ['sometimes', 'string', 'max:50000'],
            'processing_method' => [$required, Rule::enum(ProcessingMethod::class)],
            'roast_level' => [$required, Rule::enum(RoastLevel::class)],
            'arabica_percentage' => [$required, 'integer', 'min:0', 'max:100'],
            'tasting_notes' => [$required, 'array', 'max:30'],
            ...
            'gallery_media_ids.*' => ['string', 'max:200'],
        ];
    }
}
