...
final class UpdateRoasteryRequest extends FormRequest
{
    ...
    {
        $roasteryId = (string) $this->route('roasteryId');

        return [
            'name' => ['sometimes', 'string', 'min:1', 'max:160'],
            'slug' => ['sometimes', 'string', 'min:1', 'max:180', 'regex:/^[A-Za-z0-9\x{0600}-\x{06FF}](?:[A-Za-z0-9\x{0600}-\x{06FF}_-]{0,178}[A-Za-z0-9\x{0600}-\x{06FF}])?$/u', Rule::unique('roasteries', 'slug')->ignore($roasteryId)],
            ...
            'logo_media_id' => ['nullable', 'string', 'max:200'],
            'cover_media_id' => ['nullable', 'string', 'max:200'],
        ];
    }
}
