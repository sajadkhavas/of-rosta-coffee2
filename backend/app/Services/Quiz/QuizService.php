...
final class QuizService
{
    ...
    public function sync(User $user, string $id, string $guestToken, string $idempotencyKey, Request $request): QuizAttempt
    {
        return DB::transaction(function () use ($user, $id, $guestToken, $idempotencyKey, $request): QuizAttempt {
            ...
            if ($attempt->user_id !== null) {
                if ($attempt->user_id !== $user->id) { abort(404); }
                return $attempt->load('version');
            }
            $this->assertGuestToken($attempt, $guestToken);
    ...
    {
        if ($version->status !== 'draft') { throw new ApiDomainException('quiz.version_immutable', 'نسخه منتشرشده یا آرشیوشده قابل ویرایش نیست.', 409); }
        $payload = $this->validatedVersionPayload($input);
    ...
    {
        if ($version->status !== 'draft') { throw new ApiDomainException('quiz.version_not_draft', 'فقط نسخه draft قابل انتشار است.', 409); }
    ...
    private function validateAnswers(QuizVersion $version, array $answers): array
    {
        ...
        foreach ($version->questions as $question) {
            ...
            if (($question['type'] ?? 'single') === 'multi') {
                if (! is_array($value) || $value === [] || count($value) > (int) ($question['max_selections'] ?? 3)) { throw new ApiDomainException('quiz.answers_invalid', 'پاسخ‌های کوییز معتبر نیست.', 422); }
                $values = array_values(array_unique(array_map('strval', $value)));
                if (array_diff($values, $options) !== []) { throw new ApiDomainException('quiz.answers_invalid', 'پاسخ‌های کوییز معتبر نیست.', 422); }
                $normalized[$key] = $values;
            } else {
                if (! is_string($value) || ! in_array($value, $options, true)) { throw new ApiDomainException('quiz.answers_invalid', 'پاسخ‌های کوییز معتبر نیست.', 422); }
    ...
    private function score(QuizVersion $version, array $answers, Product $product): array
    {
        ...
        if ($roast !== 'recommend' && $product->roast_level->value === $roast) { $score += (int) ($weights['roast_exact'] ?? 0); $reasons[] = 'سطح رست با انتخاب شما هماهنگ است.'; }
        $brew = (string) ($answers['brew_method'] ?? '');
        if (in_array($product->roast_level->value, $rules['brew_roast'][$brew] ?? [], true)) { $score += (int) ($weights['brew_roast'] ?? 0); $reasons[] = 'برای روش دم‌آوری انتخابی شما سازگاری خوبی دارد.'; }
        $adventure = (string) ($answers['adventure'] ?? '');
        if (in_array($product->processing_method->value, $rules['adventure_process'][$adventure] ?? [], true)) { $score += (int) ($weights['process'] ?? 0); $reasons[] = 'روش فرآوری با میزان تمایل شما به طعم‌های متفاوت هماهنگ است.'; }
        foreach (($answers['flavors'] ?? []) as $flavor) {
            foreach (($version->scoring_profile['flavor_note_map'][$flavor] ?? []) as $target) {
                if (collect($product->tasting_notes ?? [])->contains(fn ($note): bool => str_contains((string) $note, (string) $target) || str_contains((string) $target, (string) $note))) {
                    $score += (int) ($weights['flavor_note'] ?? 0); $reasons[] = 'یادداشت‌های طعمی با یکی از انتخاب‌های شما هم‌پوشانی دارد.'; break;
                }
            }
        }
        ...
        if (($experienceRule['roast_level'] ?? null) === $product->roast_level->value || (isset($experienceRule['processing_not']) && $experienceRule['processing_not'] !== $product->processing_method->value)) {
            $score += (int) ($weights['experience'] ?? 0); $reasons[] = 'با سطح تجربه‌ای که انتخاب کرده‌اید هم‌خوان است.';
        }
    ...
    {
        foreach (['title', 'questions', 'scoring_profile', 'recommendation_rules'] as $key) { if (! array_key_exists($key, $input)) { throw new ApiDomainException('quiz.version_invalid', 'تعریف نسخه کوییز کامل نیست.', 422); } }
        if (! is_string($input['title']) || trim($input['title']) === '' || ! is_array($input['questions']) || count($input['questions']) < 1 || count($input['questions']) > 20 || ! is_array($input['scoring_profile']) || ! is_array($input['recommendation_rules'])) { throw new ApiDomainException('quiz.version_invalid', 'تعریف نسخه کوییز معتبر نیست.', 422); }
        $keys = [];
        foreach ($input['questions'] as $question) {
            if (! is_array($question) || ! isset($question['key'], $question['title'], $question['type'], $question['options']) || ! in_array($question['type'], ['single', 'multi'], true) || ! is_array($question['options']) || $question['options'] === []) { throw new ApiDomainException('quiz.version_invalid', 'ساختار سؤال معتبر نیست.', 422); }
            $keys[] = (string) $question['key'];
        }
        if (count($keys) !== count(array_unique($keys))) { throw new ApiDomainException('quiz.version_invalid', 'کلید سؤال‌ها باید یکتا باشد.', 422); }
    ...
    private function checksum(array $payload): string { return hash('sha256', json_encode($payload, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES)); }
    private function hashToken(string $token): string { if (strlen($token) < 32 || strlen($token) > 200) { throw new ApiDomainException('quiz.guest_token_invalid', 'شناسه مهمان معتبر نیست.', 422); } return hash('sha256', $token); }
    private function assertGuestToken(QuizAttempt $attempt, string $token): void { if ($attempt->guest_token_hash === null || ! hash_equals($attempt->guest_token_hash, $this->hashToken($token))) { abort(404); } }
}
