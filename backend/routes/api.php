...
Route::prefix('v1')->middleware('throttle:api')->group(function (): void {
    ...
    Route::post('/auth/otp/request', RequestOtpController::class)->middleware('throttle:otp-request')->name('api.v1.auth.otp.request');
    Route::post('/auth/otp/verify', VerifyOtpController::class)->middleware('throttle:otp-verify')->name('api.v1.auth.otp.verify');
    Route::get('/products', [ProductController::class, 'index'])->name('api.v1.products.index');
    Route::get('/products/{slug}', [ProductController::class, 'show'])->where('slug', '[A-Za-z0-9\p{Arabic}_-]+')->name('api.v1.products.show');
    ...
    Route::get('/content/{slug}', [ContentController::class, 'show'])->where('slug', '[A-Za-z0-9\p{Arabic}_-]+')->name('api.v1.content.show');
    Route::get('/seo/indexable', [SeoController::class, 'indexable'])->name('api.v1.seo.indexable');
    Route::get('/seo/redirects/resolve', [SeoController::class, 'redirect'])->name('api.v1.seo.redirects.resolve');
    Route::post('/cart/validate', CartController::class)->middleware('throttle:cart-validate')->name('api.v1.cart.validate');
    ...
    Route::middleware(['auth:sanctum', 'rosta.session'])->group(function (): void {
        ...
        Route::get('/me/addresses', [AddressController::class, 'index'])->name('api.v1.addresses.index');
        ...
        Route::get('/orders/{orderId}', [OrderController::class, 'show'])->where('orderId', '[A-Za-z0-9._:-]+')->name('api.v1.orders.show');
        Route::post('/orders/{orderId}/cancel', [OrderController::class, 'cancel'])->where('orderId', '[A-Za-z0-9._:-]+')->name('api.v1.orders.cancel');
        Route::get('/seller/origins', SellerOriginController::class)->middleware('rosta.role:roastery_owner,roastery_manager,roastery_staff,administrator')->name('api.v1.seller.origins.index');
        Route::post('/seller/roasteries', [SellerRoasteryController::class, 'store'])->name('api.v1.seller.roasteries.store');
        ...
        Route::prefix('/seller/roasteries/{roasteryId}')
            ...
            ->group(function (): void {
                ...
                Route::get('/media', [SellerMediaController::class, 'index'])->name('api.v1.seller.media.index');
                Route::post('/media', [SellerMediaController::class, 'store'])->name('api.v1.seller.media.store');
                Route::get('/products', [SellerProductController::class, 'index'])->name('api.v1.seller.products.index');
                Route::post('/products', [SellerProductController::class, 'store'])->name('api.v1.seller.products.store');
                Route::get('/products/{productId}', [SellerProductController::class, 'show'])->where('productId', '[A-Za-z0-9._:-]+')->name('api.v1.seller.products.show');
                ...
                Route::post('/variants/{variantId}/stock-adjustments', [SellerInventoryController::class, 'adjust'])->where('variantId', '[A-Za-z0-9._:-]+')->name('api.v1.seller.inventory.adjust');
                Route::get('/orders', [SellerOrderController::class, 'index'])->name('api.v1.seller.orders.index');
                Route::get('/orders/{orderId}', [SellerOrderController::class, 'show'])->where('orderId', '[A-Za-z0-9._:-]+')->name('api.v1.seller.orders.show');
            });

        Route::prefix('/admin')->middleware('rosta.role:administrator')->group(function (): void {
            Route::get('/origins', [AdminOriginController::class, 'index'])->name('api.v1.admin.origins.index');
            Route::post('/origins', [AdminOriginController::class, 'store'])->name('api.v1.admin.origins.store');
            ...
            Route::get('/orders/{orderId}', [AdminOrderController::class, 'show'])->where('orderId', '[A-Za-z0-9._:-]+')->name('api.v1.admin.orders.show');
            Route::get('/content-authors', [AdminContentAuthorController::class, 'index'])->name('api.v1.admin.content_authors.index');
            Route::post('/content-authors', [AdminContentAuthorController::class, 'store'])->name('api.v1.admin.content_authors.store');
            Route::patch('/content-authors/{authorId}', [AdminContentAuthorController::class, 'update'])->where('authorId', '[A-Za-z0-9._:-]+')->name('api.v1.admin.content_authors.update');
            Route::get('/content-link-report', AdminContentLinkReportController::class)->name('api.v1.admin.content_link_report');
            Route::get('/content', [AdminContentController::class, 'index'])->name('api.v1.admin.content.index');
            Route::post('/content', [AdminContentController::class, 'store'])->name('api.v1.admin.content.store');
            Route::get('/content/{entryId}', [AdminContentController::class, 'show'])->where('entryId', '[A-Za-z0-9._:-]+')->name('api.v1.admin.content.show');
            Route::patch('/content/{entryId}', [AdminContentController::class, 'update'])->where('entryId', '[A-Za-z0-9._:-]+')->name('api.v1.admin.content.update');
            Route::patch('/content/{entryId}/status', [AdminContentController::class, 'setStatus'])->where('entryId', '[A-Za-z0-9._:-]+')->name('api.v1.admin.content.status');
            Route::get('/seo-redirects', [AdminSeoRedirectController::class, 'index'])->name('api.v1.admin.seo_redirects.index');
            Route::post('/seo-redirects', [AdminSeoRedirectController::class, 'store'])->name('api.v1.admin.seo_redirects.store');
            Route::patch('/seo-redirects/{redirectId}', [AdminSeoRedirectController::class, 'update'])->where('redirectId', '[A-Za-z0-9._:-]+')->name('api.v1.admin.seo_redirects.update');
        });
    });
});
