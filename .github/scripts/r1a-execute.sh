...
sha256sum package.json bun.lock | tee "$EVIDENCE_DIR/hashes-start.txt"
...
for (const name of ['vite.config.ts', 'vite.config.js', 'vite.config.mjs', 'eslint.config.js', 'eslint.config.mjs', 'eslint.config.cjs', 'postcss.config.js', 'tailwind.config.ts']) {
...
for (const file of files) {
  ...
  for (const pattern of patterns) {
    ...
    for (const match of source.matchAll(pattern)) {
      const spec = match[1];
      if (!spec || spec.startsWith('.') || spec.startsWith('/') || spec.startsWith('@/') || spec.startsWith('virtual:') || spec.startsWith('#') || builtins.has(spec)) continue;
...
fs.writeFileSync(process.env.EVIDENCE_DIR + '/direct-import-audit.json', JSON.stringify(report, null, 2) + '\n');
console.log(JSON.stringify(report, null, 2));
if (JSON.stringify(missing) !== JSON.stringify(['@tanstack/zod-adapter'])) process.exit(20);
NODE
EVIDENCE_DIR="$EVIDENCE_DIR" node /tmp/r1a-import-audit.mjs
...
if (proof.versions['react-day-picker'] !== '9.14.0') process.exit(31);
if (proof.versions['react-resizable-panels'] !== '4.6.5') process.exit(32);
if (proof.versions['@tanstack/zod-adapter'] !== '1.167.0') process.exit(33);
...
test -z "$(git diff --name-only -- src tests scripts backend deploy .github public 2>/dev/null || true)"
