...
const required = [
  ...
  [
    ...
    "R3C2 must exercise the non-production payment boundary.",
  ],
  ["workflow", "PAYMENT_DRIVER: testing", "R3C2 must select only the testing payment provider."],
  [
    "workflow",
    "PAYMENT_CALLBACK_URL: http://127.0.0.1:3000/checkout",
    "R3C2 payment callback must return to the isolated SSR runtime.",
  ],
  [
    "workflow",
    "ROSTA_ALLOWED_PAYMENT_REDIRECT_HOSTS: 127.0.0.1",
    "R3C2 must explicitly allow only the local payment callback host.",
  ],
  ...
  [
    ...
    "All R3C2 roles must authenticate through the real one-time CLI bridge.",
  ],
  ["panelLayout", "Outlet", "The seller panel parent must render nested seller routes."],
  [
    "panelIndex",
    'createFileRoute("/panel/")',
    "Daily seller operations must live on the panel index route.",
  ],
  [
    "panelManage",
    'createFileRoute("/panel/manage")',
    "Catalog management must remain an independently rendered nested route.",
  ],
  ["otpProvider", "environment('testing')", "The expanded OTP rate must remain testing-only."],
  [
    "otpProvider",
    "otpRequestsPerMinute = $acceptanceOtp ? 12 : 3",
    "Production OTP throttling must remain unchanged.",
  ],
  [
    "paymentManager",
    "'testing' => ! app()->environment('production')",
    "The testing payment provider must remain forbidden in production.",
  ],
...
await writeFile("r3c2-commerce-roles-audit.json", `${JSON.stringify(report, null, 2)}\n`, "utf8");
